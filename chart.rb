...
dbi.select_all("select date(items.date) as date,sources.collection from items left join sources on sources.rss=items.rss where date > now() - interval '7 days' order by date") { |date,collection|
  c.add_one(collection, date.day)
}

g = Gruff::Line.new(400)
g.title = "Harvested items"
